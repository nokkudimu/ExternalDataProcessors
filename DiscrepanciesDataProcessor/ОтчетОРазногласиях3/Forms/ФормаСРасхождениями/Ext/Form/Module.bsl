...
Процедура ЗаполнитьТЧ()
	...
	Если ЗначениеЗаполнено(Объект.РасходникФСР) И ЗначениеЗаполнено(Объект.ПриходникФСР) Тогда
		...
		ТЗ_Расходник = Объект.РасходникФСР.ПолучитьОбъект().ОтгружаемыеТовары.Выгрузить();
		ТЗ_Расходник.Сортировать("Номенклатура,Характеристика ВОЗР");
		ТЗ_Приходник = Объект.ПриходникФСР.ПолучитьОбъект().Товары.Выгрузить();
		ТЗ_Приходник.Сортировать("Номенклатура,Характеристика ВОЗР");
		Результат = СравнитьНоменклатуруРасходПриход(ТЗ_Расходник, ТЗ_Приходник);
		
		Если Результат = Неопределено Тогда
			
			// проверка, есть ли акт о расхождении
			Если ЗначениеЗаполнено(Объект.АктОРасхожденииФСР) Тогда
				
				Для каждого СтрТЧ_Акт из Объект.АктОРасхожденииФСР.Товары Цикл
					
					СтрТЧ = Объект.РазногласияФСР.Добавить();
					СтрТЧ.Движения = Истина;
					СтрТЧ.Артикул = СтрТЧ_Акт.Номенклатура.Артикул;
					СтрТЧ.Штрихкод = ПолучитьШтрихкодНоменклатуры(СтрТЧ_Акт.Номенклатура, СтрТЧ_Акт.Характеристика);
					СтрТЧ.Номенклатура = СтрТЧ_Акт.Номенклатура;
					СтрТЧ.Характеристика = СтрТЧ_Акт.Характеристика;
					Если СтрТЧ_Акт.КоличествоУпаковокПоДокументу - СтрТЧ_Акт.КоличествоУпаковок > 0 Тогда
						
						СтрТЧ.Недостачи = СтрТЧ_Акт.КоличествоУпаковокПоДокументу - СтрТЧ_Акт.КоличествоУпаковок;
						
					Иначе
						
						СтрТЧ.Излишки = СтрТЧ_Акт.КоличествоУпаковок  - СтрТЧ_Акт.КоличествоУпаковокПоДокументу;
